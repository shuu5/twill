...
- **THEN** WARNING ログを出力し、health-report を生成しなければならない（SHALL）。health-check.sh 自身が Worker の停止やステータス変更を行ってはならない（MUST NOT）。ただし orchestrator が health-check 検知 + nudge 上限到達を判定した場合に限り、orchestrator が `status=failed` に遷移してよい（MAY）
