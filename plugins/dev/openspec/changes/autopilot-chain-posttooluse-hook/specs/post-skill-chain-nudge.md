...
7. 次ステップが存在する場合は `[chain-continuation] 次は /dev:<next_step> を Skill tool で実行せよ。停止するな。` を stdout に出力する
...
- **THEN** hook が stdout に `[chain-continuation] 次は /dev:<next_step> を Skill tool で実行せよ。停止するな。` を出力し、LLM コンテキストに注入される
