...
- **THEN** `_nudge_command_for_pattern` は `return 1` を返し、`/dev:workflow-test-ready` を送信しない
...
- **THEN** `_nudge_command_for_pattern` は `/dev:workflow-test-ready #N` を返す
...
- **THEN** test double は `/dev:workflow-pr-cycle #N` を返す（test-ready 以外のパターンは従来通り）
