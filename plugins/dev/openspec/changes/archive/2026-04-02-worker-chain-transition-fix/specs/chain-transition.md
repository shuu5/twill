...
- **THEN** IS_AUTOPILOT=true の場合、自動的に `/dev:workflow-pr-cycle` を Skill tool で実行する
...
- **THEN** `## チェックポイント（MUST）` セクションに従い `/dev:opsx-apply` を Skill tool で自動実行する
...
- **THEN** 即座に `/dev:workflow-pr-cycle --spec <change-id>` を Skill tool で実行しなければならない（MUST）
