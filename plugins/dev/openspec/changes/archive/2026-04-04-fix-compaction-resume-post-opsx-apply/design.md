...
`workflow-test-ready` Step 4 は「opsx-apply 実行 → IS_AUTOPILOT 判定 → pr-cycle 遷移」を一連の LLM 指示として記述している。`opsx-apply` は `/dev:apply` を通じて全タスクを実装する長時間操作であり、実行中にコンテキスト compaction が発生する可能性が高い。
...
for step in change-id-resolve test-scaffold check opsx-apply post-opsx-apply; do
  ...
  case "$step" in
    opsx-apply)
      # /dev:opsx-apply <change-id> を Skill tool で実行（Step 4 の手順を再実行）
      ;;
    post-opsx-apply)
...
- **opsx-apply の冪等性**: `current_step=opsx-apply` で compaction 後に re-run する場合、opsx-apply が途中状態で止まっていれば再実行は適切。完了していれば再実行は無駄だが、`/dev:apply` は既存実装を確認して追加適用するため大きな問題はない
