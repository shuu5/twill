...
現行 `scripts/worktree-delete.sh` はそのまま残し、新規 `commands/worktree-delete.md` を作成。COMMAND.md は script を呼び出すラッパーとして機能する。deps.yaml では command と script の両方を定義し、command が script を calls する関係を明示。
