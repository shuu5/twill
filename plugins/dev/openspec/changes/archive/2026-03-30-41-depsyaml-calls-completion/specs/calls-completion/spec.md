...
- **WHEN** co-autopilot が `/dev:workflow-setup` を Skill tool で起動する
...
- **WHEN** command:worktree-create の末尾に「`/dev:project-board-status-update` を Skill tool で自動実行」と記載されている
