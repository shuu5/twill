...
- 状態ファイルをリポジトリ別に名前空間化
- Worker が正しいリポジトリの Pilot が作成した worktree ディレクトリで起動
...
1. bare repo 構造の場合: Pilot が `{path}/worktrees/{branch}/` に事前作成した worktree ディレクトリで起動
2. standard repo の場合: Pilot が `{path}/{branch}/` に事前作成した worktree ディレクトリで起動

**理由**: 各リポジトリは独立した bare repo 構造を持つため、Worker はそのリポジトリの Pilot が作成した worktree ディレクトリで Claude Code を起動する必要がある。ADR-008 により worktree 作成は Pilot が担当する。
...
Worker が別リポジトリの Pilot が作成した worktree ディレクトリで起動されるため、git status/diff 等がそのリポジトリのコンテキストで動作する。Pilot との通信（状態ファイル）は `.autopilot/` 経由なので、AUTOPILOT_DIR を Pilot 側のパスに固定する必要がある。
