...
| パターン | 次コマンド |
|---------|-----------|
| `setup chain 完了` | `/dev:workflow-test-ready #N` |
| `>>> 提案完了` | 空（chain 内遷移） |
| `テスト準備.*完了` | `/dev:workflow-pr-cycle #N` |
| `PR サイクル.*完了` | 空（chain 終端） |
| `workflow-test-ready.*で次に進めます` | `/dev:workflow-test-ready #N` |

#### Scenario: setup chain 完了パターン検知時の nudge 送信
- **WHEN** tmux 出力に "setup chain 完了" が含まれており、出力ハッシュが変化していない
- **THEN** `/dev:workflow-test-ready #N` が Worker プロンプトに送信される（N は issue 番号）

#### Scenario: テスト準備完了パターン検知時の nudge 送信
- **WHEN** tmux 出力に "テスト準備.*完了" が含まれており、出力ハッシュが変化していない
- **THEN** `/dev:workflow-pr-cycle #N` が Worker プロンプトに送信される（N は issue 番号）
...
- **WHEN** 次コマンドが `/dev:workflow-test-ready #N` であり、issue=129
- **THEN** `/dev:workflow-test-ready #129` が Worker プロンプトに送信される
