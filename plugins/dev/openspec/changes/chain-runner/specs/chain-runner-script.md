...
- **THEN** `✓ board-status-update: Project Board Status → In Progress (#119)` のような出力が stdout に出力される
...
ISSUE_NUM を引数で受け取り、Project Board の Status を "In Progress" に更新しなければならない（SHALL）。ISSUE_NUM が未設定の場合は何も出力せず正常終了しなければならない（MUST）。

#### Scenario: Issue 番号ありで正常更新
