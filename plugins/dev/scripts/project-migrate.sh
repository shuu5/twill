...
show_help() {
    echo "使用方法: /dev:project-migrate [--type <type>] [--dry-run]"
    echo ""
    echo "オプション:"
    ...
    echo ""
    echo "例:"
    echo "  /dev:project-migrate --dry-run     # 変更内容を確認"
    echo "  /dev:project-migrate --type rnaseq # rnaseqタイプとして移行"
}
...
if [ -d ".git" ] && [ -d ".git/worktrees" ]; then
    if [ -d "main" ] && [ -f "main/.git" ]; then
        ...
        echo "  cd main && /dev:project-migrate"
        exit 1
    fi
elif [ -f ".git" ]; then
    IS_WORKTREE=true
...
echo "  /dev:controller-setup  # 開発開始"
