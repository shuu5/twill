...
# Scenario: merge-gate が PASS 時に done へ遷移 (line 15)
# WHEN: BLOCKING findings が 0 件のとき
# THEN: bash scripts/state-write.sh --type issue --issue "${ISSUE_NUM}" --role pilot --set "status=done"
#       および --set "merged_at=..." が実行される

test_merge_gate_pass_done_named_flags() {
  ...
  assert_file_contains "$MERGE_GATE_CMD" \
    '--set\s+["\x27]?status=done' || return 1
  return 0
}
run_test "merge-gate PASS 時: state-write.sh --type issue --role pilot --set status=done" \
  test_merge_gate_pass_done_named_flags

test_merge_gate_pass_merged_at_named_flags() {
  assert_file_exists "$MERGE_GATE_CMD" || return 1
  assert_file_contains "$MERGE_GATE_CMD" \
    '--set\s+["\x27]?merged_at=' || return 1
  return 0
}
run_test "merge-gate PASS 時: --set merged_at=... が実行される" \
  test_merge_gate_pass_merged_at_named_flags

# Edge case: merge-gate PASS パスで --role pilot が使われていることを確認
test_merge_gate_pass_uses_pilot_role() {
  assert_file_exists "$MERGE_GATE_CMD" || return 1
  # done への遷移は pilot ロールのみ（spec 定義）
  assert_file_contains "$MERGE_GATE_CMD" \
    '--role\s+pilot' || return 1
  return 0
}
run_test "merge-gate [edge: PASS 時の状態遷移で --role pilot が使われている]" \
  test_merge_gate_pass_uses_pilot_role
...
test_merge_gate_squash_merge_command() {
  assert_file_exists "$MERGE_GATE_CMD" || return 1
  assert_file_contains "$MERGE_GATE_CMD" \
    'gh\s+pr\s+merge\s+.*--squash' || return 1
  return 0
}
run_test "merge-gate: gh pr merge ... --squash コマンドが存在する" \
  test_merge_gate_squash_merge_command
...
test_merge_gate_worktree_delete_with_branch_name() {
  assert_file_exists "$MERGE_GATE_CMD" || return 1
  # worktree-delete.sh がブランチ名変数で呼ばれている
  assert_file_contains "$MERGE_GATE_CMD" \
    'worktree-delete\.sh\s+["\x27]?\$\{?BRANCH\}?' || return 1
  return 0
}
run_test "merge-gate: worktree-delete.sh に \${BRANCH} ブランチ名が渡されている" \
  test_merge_gate_worktree_delete_with_branch_name
