...
test_project_create_name_param() {
  ...
  assert_file_contains "$PROJECT_CREATE" '--name' || return 1
  return 0
}
run_test "project-create.sh に --name パラメータ処理がある" test_project_create_name_param

test_project_create_template_param() {
  assert_file_exists "$PROJECT_CREATE" || return 1
  assert_file_contains "$PROJECT_CREATE" '--template' || return 1
  return 0
}
run_test "project-create.sh に --template パラメータ処理がある" test_project_create_template_param
...
test_project_create_name_collision() {
  assert_file_exists "$PROJECT_CREATE" || return 1
  assert_file_contains "$PROJECT_CREATE" '(exist|already|overwrite|collision|conflict)' || return 1
  return 0
}
...
test_project_migrate_project_dir_param() {
  ...
  assert_file_contains "$PROJECT_MIGRATE" '--project-dir' || return 1
  return 0
}
run_test "project-migrate.sh に --project-dir パラメータ処理がある" test_project_migrate_project_dir_param
