...
test_owner_validation_pattern() {
  ...
  assert_file_contains "$LAUNCH_CMD" \
    'ISSUE_REPO_OWNER.*\[a-zA-Z0-9_-\]' || return 1
  return 0
}
...
test_name_validation_pattern() {
  assert_file_exists "$LAUNCH_CMD" || return 1
  assert_file_contains "$LAUNCH_CMD" \
    'ISSUE_REPO_NAME.*\[a-zA-Z0-9_\.' || return 1
  return 0
}
...
run_test "[edge] バリデーション失敗時に return 1 が存在する" \
  test_owner_validation_returns_1
...
test_repo_env_quoted() {
  assert_file_exists "$LAUNCH_CMD" || return 1
  assert_file_contains "$LAUNCH_CMD" \
    "printf\s+'%q'.*ISSUE_REPO_OWNER|printf\s+'%q'.*ISSUE_REPO_NAME" || return 1
  return 0
}
