...
LOOM_BIN="${LOOM_BIN:-/home/shuu5/.local/bin/loom}"
...
# =============================================================================
# Requirement: co-autopilot に autopilot-plan calls 宣言を追加
# Scenario: loom orphans で autopilot-plan が Isolated でなくなる (spec.md line 11)
# =============================================================================
echo ""
echo "--- Scenario: loom orphans で autopilot-plan が Isolated でなくなる ---"

# WHEN: `loom orphans` を実行する
# THEN: `script:autopilot-plan` が Isolated リストに含まれない

test_loom_orphans_autopilot_plan_is_standalone() {
  # controller→script 型制約により calls 不可のため、Isolated のまま。standalone コメントで意図明示
  local output
  output=$(cd "${PROJECT_ROOT}" && "${LOOM_BIN}" orphans 2>&1) || return 1
...
run_test "loom orphans: autopilot-plan は Isolated だが standalone コメントで意図明示" test_loom_orphans_autopilot_plan_is_standalone

# Edge case: loom orphans 出力フォーマット変化への耐性
# "Isolated" セクションヘッダーが存在することを前提に確認
test_loom_orphans_output_has_isolated_section() {
  local output
  output=$(cd "${PROJECT_ROOT}" && "${LOOM_BIN}" orphans 2>&1) || return 1
  if echo "${output}" | grep -qiP "Isolated|orphan|unused"; then
    return 0
  fi
  echo "loom orphans output has no recognizable section header (format may have changed)" >&2
  return 1
}
run_test "[edge: 出力フォーマット] loom orphans に Isolated/orphan セクションが存在する" test_loom_orphans_output_has_isolated_section
...
test_project_specific_commands_standalone_comment() {
  ...
  for cmd in loom-validate services schema-update; do
...
# Scenario: loom check が PASS する (spec.md line 57)
# WHEN: `loom check` を実行する
# THEN: すべてのチェックが PASS する

test_loom_check_passes() {
  local output
  output=$(cd "${PROJECT_ROOT}" && "${LOOM_BIN}" check 2>&1)
  local exit_code=$?
  if [[ $exit_code -ne 0 ]]; then
    echo "loom check failed:" >&2
    echo "${output}" >&2
    return 1
  fi
  # Missing: 0 を確認
  if echo "${output}" | grep -qP "Missing:\s*[1-9]"; then
    echo "loom check reports missing files:" >&2
    echo "${output}" >&2
    return 1
  fi
  return 0
}
run_test "loom check: すべてのファイルチェックが PASS する" test_loom_check_passes

# Scenario: loom validate が PASS する (spec.md line 61)
# WHEN: `loom validate` を実行する
# THEN: violations が 0 件である

test_loom_validate_passes() {
  local output
  output=$(cd "${PROJECT_ROOT}" && "${LOOM_BIN}" validate 2>&1)
  local exit_code=$?
  if [[ $exit_code -ne 0 ]]; then
    echo "loom validate failed:" >&2
    echo "${output}" >&2
    return 1
  fi
  # Violations: 0 を確認
  if echo "${output}" | grep -qP "Violations:\s*[1-9]"; then
    echo "loom validate reports violations:" >&2
    echo "${output}" >&2
    return 1
  fi
  return 0
}
run_test "loom validate: violations が 0 件である" test_loom_validate_passes
