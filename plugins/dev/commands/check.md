...
| 片方のみ | **WARN**: 不足項目を報告、`/dev:project-governance --update` を提案 |
| 両方なし | **WARN**: ガバナンス未適用、`/dev:co-project` を提案 |
...
| 両方なし | **WARN**: スキーマ未初期化（`/dev:project-governance --update` を提案） |
...
| 項目 | FAIL理由 | 対応 |
|------|---------|------|
| OpenSpec | proposal未作成 | `/dev:opsx-propose`を実行 |
...
| ガバナンス | 未適用 | `/dev:co-project` を提案 |
| スキーマ | 未初期化 | `/dev:project-governance --update` を提案 |

**重要**: FAIL項目がある場合、`/dev:workflow-pr-cycle` への進行をブロック。

## PRサイクル開始条件

以下をすべて満たす場合のみ `/dev:workflow-pr-cycle` を推奨:
...
- `/dev:workflow-setup` - 統合開発ワークフロー（推奨入口）
- `/dev:workflow-setup` + `/dev:workflow-test-ready` - 開発準備
- `/dev:workflow-pr-cycle` - PRサイクル実行
