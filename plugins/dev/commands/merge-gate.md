...
#### autopilot 時（Worker が実行）

status が `running` または `merge-ready` の場合、Worker は merge-ready を宣言して停止する。
...
#### 非 autopilot 時（Pilot が実行）

status が空または `running`/`merge-ready` 以外の場合、Pilot が merge-gate-execute.sh 経由でマージを実行する。
