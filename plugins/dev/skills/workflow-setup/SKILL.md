...
**軽量 chain（is_quick=true）**:
- IS_AUTOPILOT=true → 直接実装後 commit → push → `gh pr create --fill --label quick` → merge-gate（pr-cycle スキップ）。即座に実行せよ。プロンプトで停止するな。
- IS_AUTOPILOT=false → 「直接実装 → merge-gate で完了可」と案内。
