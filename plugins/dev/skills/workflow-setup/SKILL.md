...
`commands/worktree-create/COMMAND.md` を Read → 指示に従い実行。
init の `recommended_action` が `worktree` の場合のみ。

### Step 2.3: project-board-status-update（Project Board 更新）
ISSUE_NUM がある場合のみ。`commands/project-board-status-update/COMMAND.md` を Read → 実行。

### Step 2.4: crg-auto-build（CRG グラフビルド）
`commands/crg-auto-build/COMMAND.md` を Read → 実行。
...
`commands/opsx-propose/COMMAND.md` を Read → 実行。

### Step 3.5: ac-extract（AC 抽出）
`commands/ac-extract/COMMAND.md` を Read → 実行。

### Step 4: workflow-test-ready へ遷移
`--auto` 時: 自動的に `/dev:workflow-test-ready` を Skill tool で実行して chain を継続。
通常時: 「setup chain 完了。`/dev:workflow-test-ready` で次に進めます」と案内。
