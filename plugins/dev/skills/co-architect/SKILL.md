...
→ `/dev:architect-group-refine <context-name>` を実行して終了（Step 1〜8 スキップ）。
...
`/dev:explore` を Skill tool で呼び出す。以下をコンテキストとして注入:
...
`/dev:architect-completeness-check` を実行。
...
次のステップ:
  - autopilot 実行: /dev:co-autopilot で Issue 群を一括実装
  - 個別実装: /dev:workflow-setup #N で1件ずつ実装
```
