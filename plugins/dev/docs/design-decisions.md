...
- loom CLI の登場により、構造チェックを外部ツールに委譲可能になった

### 新設計（動的構築）

loom-plugin-dev では変更ファイルから reviewer を動的に構築。パス分岐を廃止し、
全レビューを並列 Task spawn で実行。loom validate/check が構造チェックを担当。
...
loom-plugin-dev でも不変条件 H を維持。autopilot-plan.sh が依存グラフ構築時に
...
loom-plugin-dev では全9件を autopilot-invariants.bats で自動検証。
