...
| Where（領域） | ラベル（scope/* + ctx/*） | 変更対象のコンポーネント・Bounded Context を特定 |
...
## 2. Where 軸 — scope/* + ctx/* ラベル

### scope/* ラベル（モノリポコンポーネント分類）

Where 軸は 2 レイヤーで構成する:
- **scope/***: モノリポのコンポーネント（必須。全 Issue に1つ付与）
- **ctx/***: コンポーネント内の Bounded Context（architecture/domain/contexts/ を持つコンポーネントのみ）
...
変換規則: コンポーネントパスのスラッシュをハイフンに置換する。
...
### ctx/* ラベル（Bounded Context 分類）

#### 適用条件

architecture/domain/contexts/ ディレクトリを持つコンポーネントでのみ使用する。scope/* は全プロジェクトで使用する。
...
| 操作 | コンポーネント | 説明 |
|------|---------------|------|
| Producer | label-sync / issue-structure | ルートレベル context-map.md から導出し Issue に付与 |
| Consumer | Board Labels フィールド | フィルタ・グループ化 |
