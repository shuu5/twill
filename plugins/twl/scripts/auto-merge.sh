...
if ! gh pr ready "$PR_NUMBER" 2>"$PR_READY_ERR"; then
  READY_ERR_RAW=$(sed -E 's/gh[a-z]_[a-zA-Z0-9_]+/***MASKED***/g; s/github_pat_[a-zA-Z0-9_]+/***MASKED***/g; s/Bearer [^ ]+/Bearer ***MASKED***/g' "$PR_READY_ERR" | head -c 300)
...
if ! gh pr merge "$PR_NUMBER" --squash 2>"$MERGE_ERROR_LOG"; then
  ERROR_RAW=$(sed -E 's/gh[a-z]_[a-zA-Z0-9_]+/***MASKED***/g; s/github_pat_[a-zA-Z0-9_]+/***MASKED***/g; s/Bearer [^ ]+/Bearer ***MASKED***/g' "$MERGE_ERROR_LOG" | head -c 500)
