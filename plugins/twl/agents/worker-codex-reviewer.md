...
if command -v codex >/dev/null 2>&1 && codex login status 2>&1 | grep -qi "logged in"; then
  # model probe: fallback/ERROR を検出した場合は skip (AC #3)
  PROBE_MODEL="${TWILL_CODEX_REVIEW_MODEL:-gpt-5.1-codex}"
  PROBE_OUT=$(echo "test" | timeout 10 codex exec -m "$PROBE_MODEL" --sandbox read-only --ephemeral 2>&1 | head -5 || true)
  if ! echo "$PROBE_OUT" | grep -qi "fallback\|error\|unknown model\|invalid"; then
    CODEX_OK=1
  fi
fi
