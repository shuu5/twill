...
以下を **単一の Bash tool call** で確認する:

```bash
# codex インストール確認と認証確認
# デフォルト model: gpt-5.3-codex (OpenAI Codex Industry-leading coding model)
# 公式: https://developers.openai.com/codex/models (失効確認日: 2026-05-01)
# 旧デフォルトは 2026-05-01 時点で公式一覧から消失 → gpt-5.3-codex に更新
CODEX_OK=0
if command -v codex >/dev/null 2>&1 && codex login status 2>&1 | grep -qi "logged in"; then
  PROBE_MODEL="${TWILL_CODEX_REVIEW_MODEL:-gpt-5.3-codex}"
  PROBE_OUT=$(echo "test" | timeout 10 codex exec -m "$PROBE_MODEL" --sandbox read-only --ephemeral 2>&1 | head -20 || true)
  ...
  if [[ -f "$_PROBE_CHECK_SCRIPT" ]]; then
    ...
    source "$_PROBE_CHECK_SCRIPT"
    run_probe_check
  elif ! echo "$PROBE_OUT" | grep -qi "fallback\|error\|unknown model\|invalid"; then
    CODEX_OK=1
  fi
fi
...
**`CODEX_OK=0` の場合**（codex 未インストール・認証未設定・model probe 失敗のいずれか）はスキップ（graceful skip）し、以下を出力して即完了する（エラーメッセージは出力しない）:

silent skip（reason なし）は禁止。`CODEX_SKIP_REASON` の値（`run_probe_check` が設定した理由）を `reason:` フィールドに必ず記録すること。
...
findings:
  - reason: "<CODEX_SKIP_REASONの値>"
```

`CODEX_SKIP_REASON` が未設定の場合は `"codex unavailable"` を使用すること。
...
# gpt-5.3-codex: Industry-leading coding model (https://developers.openai.com/codex/models, 2026-05-01)
codex exec --sandbox read-only -m "${TWILL_CODEX_REVIEW_MODEL:-gpt-5.3-codex}" --ephemeral < "$PROMPT_FILE"
```
