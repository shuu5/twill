...
- **関数シグネチャ変更時の caller クロスチェック（MUST）**: 関数・メソッドのシグネチャ変更（引数追加・削除・型変更）を検出した場合、caller 側の全参照箇所をクロスチェックすること（`caller-signature-mismatch`）。シグネチャと呼び出し側が不整合な場合は CRITICAL（confidence ≥ 90）として報告する。
  1. Grep で対象関数名を検索し、呼び出し箇所を特定する
  2. 各呼び出し箇所で引数の数・型・キーワード引数が新シグネチャと一致するか確認する
  3. 不一致が存在する場合は CRITICAL（confidence ≥ 90）として報告する

  **False-positive 除外**: デフォルト値追加（既存呼び出しが全て新シグネチャに適合する場合）および動的呼び出し（`getattr` 等、静的解析で追跡不可能な場合）は報告対象外とする。
