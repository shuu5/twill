...
E2E Scenario検出時 → `/twl:e2e-generate` を直接呼び出し → 4層検証パターン強制適用。
...
## PreToolUse Hook テストのペイロードパターンガイドライン

PreToolUse hook（`scripts/hooks/` 配下）のテストを生成する際は、以下の2パターンを**両方**カバーすること。

### パターン 1: file_content 経由（インラインペイロード）

ペイロード JSON に `tool_input.file_content` を含める方式。テストの記述が簡潔でファイルシステム非依存。
...
### パターン 2: ディスクベース（file_content 省略）

**Claude Code の実ペイロードには `file_content` フィールドは含まれない。**
実運用では hook スクリプトが `file_path` からディスク上のファイルを読み込んで検証する。

テスト手順:
1. 一時 git リポジトリを作成（`init_temp_repo`）
2. 対象ファイルを配置して `git add && git commit`
3. `file_content` を**含まない**ペイロードで hook を実行
4. 実際の YAML 検証パスを通過することを確認（exit 0 の no-op ガードに頼らない）

```json
{
  "tool_name": "Edit",
  "tool_input": {
    "file_path": "/path/to/repo/deps.yaml",
    "old_string": "置換前",
    "new_string": "置換後"
  }
}
...
ディスクベーステストでは**有効 YAML（exit 0）と不正 YAML（exit 2）の両方**を生成し、実際のファイル読み込み→YAML パース→検証の完全なパスを網羅すること。
