...
| 優先度 | 分類 | 条件 | 表示上限 |
|--------|------|------|----------|
| 1 | **解決済み候補** | tech-debt のタイトルのキーワードが `tests/` 内のテスト名または Scenario 名に部分文字列一致（大文字小文字区別なし） | 最大3件 |
...
```bash
KEYWORD="<keyword>"
grep -rli -F -- "$KEYWORD" tests/ 2>/dev/null
```

tech-debt のタイトルから抽出したキーワードを `tests/` 内で固定文字列検索（`-F`）する。`-i` で大文字小文字を区別せず、`--` でオプション終端を明示する。対応するテストが存在すれば「解決済み候補」として分類する。
