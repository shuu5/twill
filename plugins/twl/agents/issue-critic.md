...
`**/refs/ref-investigation-budget.md` を Glob/Read して調査バジェットを確認すること。
...
分類基準は `ref-specialist-output-schema.md` の「co-issue specialist 追加要件」セクションを参照。issue-critic の findings は基本的に `issue_description` が大半を占める。コードベースの状態確認が必要な場合（例: スコープファイルの存在確認）は `codebase_state` を使用すること。
