...
| `**/architecture/examples/**` パターンに一致するファイルは検証対象外（テンプレート・実例） | INFO |

**SHA 検証ロジック**: `sha:` フィールドの値を `^[a-f0-9]{40}$` で検証する。マッチしない場合（tag / branch / HEAD / 短縮 SHA を含む）は CRITICAL。`**/architecture/examples/**` glob パターンに一致するファイル（例: `plugins/twl/architecture/examples/`、`architecture/examples/`）はスキャン対象外とする。
