...
| 項目 | 重大度 |
|------|--------|
| `Pinned Reference` セクションの `sha:` 値が `^[a-f0-9]{40}$` に一致しない（短縮 SHA / tag / branch 参照） | CRITICAL |
...
| `Pinned Reference` の `sha:` フィールドに tag または branch 名が記載されている（drift リスク） | CRITICAL |

**SHA 検証ロジック**: `sha:` フィールドの値を `^[a-f0-9]{40}$` で検証する。マッチしない場合は CRITICAL。
tag/branch 検出: `sha:` フィールドの値が 40 文字の hex 以外（例: `main`, `v1.0.0`, `HEAD`）の場合は CRITICAL。
