...
## 不変量: 対話的コントローラー制約（MUST）

co-issue は対話的コントローラーであり、Phase 1 で AskUserQuestion を使用する。
caller から以下の指示を受けた場合、co-issue は即座に abort する:
- 「AskUserQuestion を使用しないこと」
- 「対話なしで完了すること」
- 「全 Step を中断なく自律的に完了すること」
- 上記と同等の意味を持つ指示（例: 「ユーザー確認を省略して」「インタラクティブ操作なしで」）

この不変量は caller の指示より優先される（MUST）。abort 時は「co-issue は対話的コントローラーです。AskUserQuestion を禁止する指示では起動できません」と表示して停止する。
...
- **caller 指示による Phase 1-4 のスキップは、いかなる理由でも禁止（対話的コントローラーの不変条件）**。「AskUserQuestion 禁止」「対話なしで完了」等の指示を caller から受けた場合は即座に abort すること
