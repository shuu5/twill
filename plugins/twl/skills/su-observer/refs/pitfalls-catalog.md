...
## 22. feature-dev spawn gate の承認証跡運用（Issue #1635 / ADR-040 文書化）

### 事象

ADR-040 で導入された `.supervisor/feature-dev-request-<N>.json` 承認証跡 + MCP tool 化により、observer が feature-dev を起動する経路は以下に集約された:

1. user 明示依頼受信
2. observer が承認証跡ファイルを作成（TTL=1800s）
3. `mcp__twl__twl_spawn_feature_dev(issue=N, ...)` 呼び出し
4. tool 内 gate 検証（schema / TTL / Status=Refined / parallel-check）
5. cld-spawn 経由で `wt-fd-<N>` window spawn
6. 承認証跡を `.supervisor/consumed/` に atomic rename（one-shot 消費）

### 主要 pitfall

#### 22.1: TTL 内に spawn しなかった場合は再依頼が必要

承認証跡作成から 30 分以内に spawn しなかった場合、TTL expired となり MCP tool は DENY を返す。observer はファイルを削除して user に再依頼を求める必要がある。

**対処**: user 依頼受信後、5 分以内に MCP tool を呼ぶ運用を推奨。逆に「依頼を受けたが他の作業で遅延しそう」な場合は、依頼受信時点ではなく **spawn 実行直前** に承認証跡を作成する。

#### 22.2: consumed/ への atomic rename が失敗した場合の手動 cleanup

POSIX 同 FS atomic rename はほぼ確実に成功するが、cross-fs（tmpfs → ext4 等）の場合は `shutil.copy2 + unlink` フォールバックする。両方が失敗した稀なケースでは:

- 承認証跡が `.supervisor/feature-dev-request-<N>.json` に残る
- consumed/ には `.consumed.partial-<N>-<ts>.json` 等が残る可能性
...
`spawn-controller.sh feature-dev` を `SKIP_LAYER2=1 SKIP_LAYER2_REASON='<reason>'` で直接呼ぶ既存 path は deprecation period 中（2 wave 後廃止予定）。stderr に DEPRECATION warning が出力される。

**対処**: warning を見たら MCP tool 経路への移行を実施。完全削除までの間は escape hatch として機能するが、運用は MCP tool 経由を優先する。

### 検出シグナル

- `/tmp/mcp-shadow-spawn-feature-dev.log` の連続 DENY (`"ok": false`) → user 依頼との不整合または承認証跡作成手順の問題
- `.supervisor/consumed/` の累積数 / 期間 → audit 用途
- spawn-controller.sh stderr に `DEPRECATION: SKIP_LAYER2=1` warning → 移行未完了の運用箇所

**参照**: Issue #1635, ADR-040 (feature-dev spawn gate), SU-10 (`supervision.md` Constraints), intervention-catalog.md パターン 14 (Layer 1 Confirm), `mcp__twl__twl_spawn_feature_dev` MCP tool 仕様
