...
2. **Status=Todo の場合は `/twl:co-issue refine #<N>` を spawn する（唯一の正規経路）**:
   ...
   ```
   - **MUST NOT**: `chain-runner.sh board-status-update <N> Refined` を co-issue Phase 4 外から直接実行してはならない（ADR-024 大原則 bypass）
   - **Emergency bypass（例外のみ）**: cross-repo Issue 等で refine 経路が成立しない場合、`--bypass-status-gate` + PR description `retroactive bypass: ADR-024 violation acknowledged, fix tracked in #<epic>` を使用
...
## feature-dev 明示依頼時の起動手順（Issue #1644 — SU-10 改訂、spawn-controller.sh 統合）
...
**重要 (Issue #1644)**: feature-dev spawn は spawn-controller.sh feature-dev path 経由に統合された。MCP tool `mcp__twl__twl_spawn_feature_dev` は spawn-controller.sh を呼ぶ薄い wrapper として維持される。承認証跡 gate + worktree 作成 + hook 設置 + skill prefix prepend + main 直接 push 防止プロンプト注入は全て spawn-controller.sh feature-dev path 内で実行される。
...
**Step 2: MCP tool 経由で spawn（wrapper として spawn-controller.sh を呼ぶ）**

```python
# observer Python セッション内 or MCP client から
result = mcp__twl__twl_spawn_feature_dev(
    issue=1644,
    worktree_path=None,  # optional: 省略時は $TWILL_ROOT/worktrees/fd-<N> を auto-create
    model="claude-opus-4-7",  # default
    timeout=120,  # default
)
# result.ok == True なら spawn 成功、result.window == "wt-fd-1644" 等
```

または直接 spawn-controller.sh を呼ぶ場合（同等）:

```bash
bash plugins/twl/skills/su-observer/scripts/spawn-controller.sh feature-dev 1644 \
  --model claude-opus-4-7 --timeout 120
```

spawn-controller.sh feature-dev path の処理チェイン:
1. ISSUE_NUMBER バリデーション（正整数）
2. 承認証跡 schema validation（`{issue, requested_at, requested_by, ttl_seconds, intervention_id}`）
3. TTL チェック (now - requested_at ≤ ttl_seconds、デフォルト 1800s)
4. `--check-refined-status <N>` self shell-out → Status=Refined のみ pass
5. `--check-parallel-only <N>` self shell-out → SU-4 並列上限チェック
6. 承認証跡を `.supervisor/consumed/feature-dev-request-<N>-<ts>.json` に atomic rename（`mv -n`、cross-fs では `cp + rm` フォールバック、one-shot 消費）
7. worktree auto-create（`--cd` 未指定時、`$TWILL_ROOT/worktrees/fd-<N>` に `git worktree add`）
8. pre-push hook 設置（`install-git-hooks.sh --worktree <PATH>`、per-worktree `core.hooksPath = .fd-hooks/`）
9. FINAL_PROMPT 構築（`/feature-dev:feature-dev #<N>` + provenance + MUST 注入）
10. cld-spawn 実行 (`wt-fd-<N>` window、`--cd <worktree>`)
...
- worktree branch: `fd-<N>`（例: `fd-1644`、spawn-controller.sh が auto-create）

### main 直接 push 防止（Issue #1644 で追加）

spawn-controller.sh feature-dev path は以下の二段で main 直接 push を防ぐ:

1. **pre-push hook** (per-worktree `core.hooksPath = .fd-hooks/`): worktree 内で `git push origin main` を実行すると hook が exit 1。bypass は `git push --no-verify`（user 裁量）
2. **prompt 注入**: skill prefix 直後に「MUST: worktree 内作業」「main への直接 push 禁止」「PR 経由 merge」を注入

### deprecated path（移行期のみ使用可）

`SKIP_LAYER2=1 SKIP_LAYER2_REASON='<reason>' spawn-controller.sh feature-dev <N>` で gate check をバイパス可能（2 wave 維持、AC-4.6）。MCP tool 経由（gate check 付き）を優先すること。

### 失敗時の対処

| エラー | 原因 | 対処 |
|---|---|---|
| `feature-dev approval trail not found` | 承認証跡未作成 | user に再依頼を求め、Step 1 から実施 |
| `approval trail TTL expired` | 承認証跡作成から TTL 超過 | 承認証跡を削除して Step 1 から再実施 |
| `approval trail missing field` | 承認証跡 schema 違反 | JSON フォーマットを確認、必須 field を補完 |
| `Status=Refined check failed` | Issue Status が Refined でない | `/twl:co-issue refine #<N>` を spawn して Refined へ遷移（唯一の正規経路。MUST NOT: chain-runner.sh board-status-update 直接実行禁止） |
| `parallel spawn denied` | SU-4 並列上限超過 | 他の supervised controller を待機 or completion |
| `approval trail already consumed by a parallel caller` | 並列呼び出しで race lost | 別 spawn が既に成功している。tmux window を確認 |
