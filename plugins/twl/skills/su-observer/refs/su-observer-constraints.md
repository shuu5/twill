...
- feature-dev plugin を `mcp__twl__twl_spawn_feature_dev` MCP tool 以外で spawn してはならない（SU-10）。user 明示依頼受信時に `.supervisor/feature-dev-request-<N>.json` 承認証跡を書き出し、MCP tool 経由で gate 検証後に spawn を実行する。SKIP_LAYER2=1 path は deprecation period 中で代替推奨
