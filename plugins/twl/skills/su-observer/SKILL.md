...
| `cld-observe-loop --pattern 'ap-*' --interval 180` | Worker 群 polling（hook 不在時フォールバック） | 3 分 |
...
| `[BUDGET-LOW]` tmux status line budget 残量パース | 5h rolling budget 枯渇前の安全停止 | 残り 15 分（`.supervisor/budget-config.json` で override 可） |

**[BUDGET-LOW] 検知・停止シーケンス（supervise iteration 内で毎回実行）:**

```bash
# status line から budget 残量を抽出（正規表現ベース。フォーマット変動に対応）
