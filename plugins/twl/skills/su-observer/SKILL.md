...
#### co-autopilot spawn 前 MUST（#1516 — Wave spawn 前 Status=Refined check）
...
1. 対象 Issue の Project Status を確認:
   ...
   ```
2. Status=Todo の場合は `board-status-update --status Refined` を実行して Refined に遷移させる（または `co-issue refine` を spawn）:
   ```bash
