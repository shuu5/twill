...
doobidoo に lesson を保存した直後に以下を全て実施するまで「完遂」と扱わない（MUST）:
1. Issue 起票 (`gh issue create` for follow-up implementation)
2. Wave 計画（TaskCreate for tracking）
3. SKILL/refs PR（Wave で実装 — pitfalls-catalog / SKILL.md / ADR への反映）
