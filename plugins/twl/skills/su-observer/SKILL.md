...
#### co-autopilot spawn 前 MUST（#1516 / #1557 — Wave spawn 前 Status=Refined check）

co-autopilot を spawn する**前に必ず**以下を実行すること（`refs/pitfalls-catalog.md §19` 参照）:

1. 対象 Issue の Project Status を確認:
   ```bash
   gh project item-list <BOARD_NUMBER> --owner <OWNER> --format json \
     | jq -r '.items[] | select(.content.number == <ISSUE_NUM>) | .status'
   ```
2. **Status=Todo の場合は `/twl:co-issue refine #<N>` を spawn して refine 経路で Refined 遷移させる（唯一の正規経路）**:
   ```bash
   bash "${CLAUDE_PLUGIN_ROOT}/scripts/spawn-controller.sh" co-issue "refine #<ISSUE_NUM>"
   ```
   - **MUST NOT**: `chain-runner.sh board-status-update <N> Refined` は **co-issue Phase 4 外からの直接実行禁止**（ADR-024 大原則 bypass、#1557 epic bypass #4 として認定済み、`TWL_CALLER_AUTHZ=co-issue-phase4` のみが授権コンテキスト）
   - **Emergency bypass（例外）**: Project Board 未登録の cross-repo Issue 等で refine 経路が成立しない場合に限り `autopilot-launch.sh` に `--bypass-status-gate` を渡す。PR description に **`retroactive bypass: ADR-024 violation acknowledged, fix tracked in #<epic>`** を併記すること（ADR-024 L60、#1557 AC8 準拠）
3. Status=Refined を確認後に co-autopilot spawn を実行する。

**なぜ MUST か**: ADR-024 Phase B により orchestrator は Status=Refined でない Issue の Worker spawn を reject する。Status=Todo のまま spawn すると Pilot が skip を繰り返す stuck pattern になる（Wave 60 / Wave 63 で 2 度発生）。**直接 `board-status-update` 経路を許可すると ADR-024 大原則（co-explore → Todo → co-issue → Refined → co-autopilot）が bypass される**ため、refine spawn を唯一経路として強制する（#1557 epic）。
