...
| `--with-observer` | （オプション） | WITH_OBSERVER=true。Observer ペア起動を有効化 |
...
## Step 3.5: Observer ペア起動（WITH_OBSERVER=true 時のみ）

`WITH_OBSERVER=true` の場合、セッション初期化（Step 3）直後に co-observer を別 tmux window で起動する。

```bash
# AUTOPILOT_WINDOW: 現在の autopilot tmux window 名
AUTOPILOT_WINDOW=$(tmux display-message -p '#W')

Skill(session:spawn) を呼び出し:
  prompt: "twl:co-observer --target $AUTOPILOT_WINDOW --session-id $SESSION_ID --autopilot-dir $AUTOPILOT_DIR --plan $AUTOPILOT_DIR/plan.yaml"
  window_name: "observer-${SESSION_ID}"
```

渡すコンテキスト（MUST）:
- `--target <autopilot-window>`: 監視対象の tmux window 名
- `--session-id <SESSION_ID>`: autopilot セッション ID（state ファイル特定用）
- `--autopilot-dir <AUTOPILOT_DIR>`: autopilot 状態ディレクトリ（issue-{N}.json, session.json）
- `--plan <plan.yaml パス>`: 実行計画ファイルのパス

Observer は autopilot 完了時（Step 5 のサマリー報告後）に自動終了する。
crash-resume 時の Observer 振る舞い: autopilot が resume した場合、Observer は同一 `--target` で再アタッチを試みる（window が存在する場合）。window が消滅していた場合は新規 spawn する。

3+ Issue 実行時に `--with-observer` を推奨（AUTO=true 時も同様）。
