...
| `--auto` | （MODEに付加） | AUTO=true（後方互換、省略時も AUTO） |
| `--interactive` | （MODEに付加） | INTERACTIVE=true（Plan 承認 menu opt-in） |
| `--repos '{"lpd":{...}}'` | （REPOSに設定） | クロスリポジトリ設定 JSON |

`PROJECT_DIR`・`REPO_MODE`・`AUTOPILOT_DIR` を自動判定し export する。

**AUTOPILOT_DIR 判定ロジック（#1169 明示化）:**
- (a) env var `AUTOPILOT_DIR` が事前 export されていれば Pilot はそのまま使用（チェックなし。basename pattern 検証は chain-runner.sh L185-191 が担う）
- (b) 未設定時、Pilot は `${PROJECT_DIR}/.autopilot` を default として export する（既存挙動）
- (c) Pilot は basename pattern 検証を**行わない**（実装層の責務）。並列実行時に呼び出し側（spawn-controller.sh 等）が `.autopilot-wave10` 等を渡す場合、検証は chain-runner.sh の AUTOPILOT_DIR 信頼性検証と auto-merge.sh の glob 走査で担保される
...
## Step 2: 計画承認
デフォルト AUTO（★HUMAN GATE なし）— `--interactive` opt-in 時のみ AskUserQuestion で Phase 構成をユーザーに確認

`--interactive` なし（default）→ 自動承認（Phase 1 直行）。`--interactive` 時 → ★HUMAN GATE: AskUserQuestion で Phase 構成を表示し確認。
...
### Worker auto mode 確認方針（observer 観察用）
...
- Worker bash で permission prompt（`1. Yes, proceed` / `2. No, and tell ...`）が出た場合、auto mode classifier の `soft_deny` 該当（仕様）の可能性が高い → su-observer の pitfalls-catalog.md §4.7 / §4.8 で対処
...
主要不変条件: K=Pilot 実装禁止 / B=worktree ライフサイクル Pilot 専任 / F=merge-gate 失敗時 rebase 禁止 / M=chain 停止時 Pilot 直接 nudge 禁止
