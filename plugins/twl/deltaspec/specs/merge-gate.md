...
## Scenario: merge 前 base drift 検知

- **WHEN** merge-gate が PASS を返し squash merge を実行する前
- **THEN** origin/main に対する silent deletion（意図しないファイル削除）を検知する（不変条件 J）
- **AND** base drift が検出された場合、merge を停止し Pilot に報告する
- **AND** base drift が検出されない場合、squash merge を実行する

## Scenario: merge 失敗時の対応

- **WHEN** squash merge がコンフリクト等で失敗する
- **THEN** 停止のみ行い自動 rebase は行わない（不変条件 F）
- **AND** Pilot に手動介入を要求する
