...
- **THEN** IS_AUTOPILOT 判定スニペットが実行され、IS_AUTOPILOT=true であれば `/twl:workflow-pr-cycle --spec <change-id>` が Skill tool で呼び出される

#### Scenario: opsx-apply 完了後の通常フローで IS_AUTOPILOT=false の場合

- **WHEN** opsx-apply 完了後に IS_AUTOPILOT 判定が実行され、IS_AUTOPILOT=false である
- **THEN** 従来通り案内メッセージ「workflow-test-ready 完了。次のステップ: /twl:workflow-pr-cycle」を表示して停止する（動作変更なし）
