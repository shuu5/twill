...
PostToolUse hook の stdout は LLM コンテキストに直接注入される（Claude Code の仕様）。出力は `[chain-continuation] 次は /twl:<skill> を Skill tool で実行せよ。停止するな。` の単行とし、HTML/シェルインジェクションを防ぐためサニタイズする（英数字・`/:-_` のみ許可）。
