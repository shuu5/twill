...
- [x] 1.2 IS_AUTOPILOT=true 時に「即座に `/twl:workflow-test-ready` を Skill tool で実行せよ。プロンプトで停止するな」の指示を追加
...
- [x] 2.2 IS_AUTOPILOT=true 時に「即座に `/twl:workflow-pr-cycle --spec <change-id>` を Skill tool で実行せよ。プロンプトで停止するな」の指示を追加
...
- [x] 3.2 Step 3 から `/twl:workflow-pr-cycle` 呼び出しロジックを削除
...
- [x] 4.2 CRITICAL FAIL なし → `/twl:opsx-apply` を Skill tool で自動実行
