...
`autopilot-orchestrator.sh` の `_nudge_command_for_pattern` が quick ラベルを考慮せず、"setup chain 完了" パターン検出時に一律 `/twl:workflow-test-ready` を nudge として送信するため、quick Issue の Worker が不要な test-ready → pr-cycle チェーンを実行してしまう。
