...
quick + IS_AUTOPILOT=true の場合、`/twl:workflow-test-ready` を呼び出してはならない（MUST NOT）。
...
- **THEN** 従来通り即座に `/twl:workflow-test-ready` を Skill tool で実行しなければならない（SHALL）
...
- **THEN** プロンプトは現行通り `/twl:workflow-setup #${ISSUE}` のままでよい（SHALL）
