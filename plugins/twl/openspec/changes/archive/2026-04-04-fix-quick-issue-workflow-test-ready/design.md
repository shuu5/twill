...
- `workflow-setup SKILL.md` Step 4 は `IS_AUTOPILOT=true` だけを見て無条件に `/twl:workflow-test-ready` を呼び出す指示になっており、`is_quick` チェックが存在しない
...
- `autopilot-launch.sh` の `PROMPT` は常に `/twl:workflow-setup #${ISSUE}` 固定で、quick ラベル情報が Worker に渡されない
