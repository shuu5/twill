...
- Phase 1: 問題探索（/twl:explore 呼び出し、explore-summary.md 保存）
...
co-issue は Phase 4 完了後に `.controller-issue/` ディレクトリを削除しなければならない（MUST）。Issue URL を表示し、`/twl:workflow-setup #N` を案内しなければならない（SHALL）。
