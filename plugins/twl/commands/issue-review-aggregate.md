...
各 Step の入退出時に以下のフォーマットで stdout に出力すること（pane capture で観測可能にするため）:
...
ISO8601 例: `2026-05-04T01:58:11Z`（`date -u +%Y-%m-%dT%H:%M:%SZ` 相当）。

stuck 検知用 grep パターン: `grep -E '^\[AGGREGATE-STEP\] step=[0-9]+ status=(enter|exit) ts='`

## フロー（MUST）

### Step 1: 出力なし完了の検知（上位ガード）

Step 開始時: `[AGGREGATE-STEP] step=1 status=enter ts=<ISO8601>`
...
Step 完了時: `[AGGREGATE-STEP] step=1 status=exit ts=<ISO8601>`

### Step 2: findings 統合

Step 開始時: `[AGGREGATE-STEP] step=2 status=enter ts=<ISO8601>`

全 specialist の findings を Issue 別にマージする。

Step 完了時: `[AGGREGATE-STEP] step=2 status=exit ts=<ISO8601>`

### Step 3: ブロック判定

Step 開始時: `[AGGREGATE-STEP] step=3 status=enter ts=<ISO8601>`
...
Step 完了時: `[AGGREGATE-STEP] step=3 status=exit ts=<ISO8601>`

### Step 4: ユーザー提示

Step 開始時: `[AGGREGATE-STEP] step=4 status=enter ts=<ISO8601>`
...
Step 完了時: `[AGGREGATE-STEP] step=4 status=exit ts=<ISO8601>`

### Step 5: CRITICAL ブロック処理

Step 開始時: `[AGGREGATE-STEP] step=5 status=enter ts=<ISO8601>`
...
Step 完了時: `[AGGREGATE-STEP] step=5 status=exit ts=<ISO8601>`

### Step 6: split 提案ハンドリング

Step 開始時: `[AGGREGATE-STEP] step=6 status=enter ts=<ISO8601>`
...
Step 完了時: `[AGGREGATE-STEP] step=6 status=exit ts=<ISO8601>`
