...
/twl:project-board-configure --project "$(twl config get project-board.number)" --owner "$(twl config get project-board.owner)"
```
...
| 引数 | 必須 | 説明 |
|------|------|------|
| --project N | No | Project 番号（未指定: `twl config get project-board.number` で自動取得） |
| --owner NAME | No | Project オーナー（未指定: `twl config get project-board.owner` で自動取得） |
...
THEN そのまま使用
ELSE
  `twl config get project-board.number` と `twl config get project-board.owner` で取得
  IF 失敗 → エラー終了: "project-links.yaml に Project Board 情報がありません"
```
