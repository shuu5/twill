...
1. スクリプト実行:
   ```bash
   bash "${CLAUDE_PLUGIN_ROOT}/scripts/ecc-monitor.sh" check
   ```
...
3. チェックポイント保存:
   ```bash
   bash "${CLAUDE_PLUGIN_ROOT}/scripts/ecc-monitor.sh" save-checkpoint
   ```
...
4. dev pluginの現在構造を把握:
   ```bash
   ls claude/plugins/twl/commands/
   ls claude/plugins/twl/skills/
   ls claude/plugins/twl/agents/
   ```
