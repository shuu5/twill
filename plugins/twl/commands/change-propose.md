...
state の `mode` フィールドと `deltaspec/` ディレクトリの存在を確認して auto_init を判定する:
...
# deltaspec/ 存在チェック
DELTASPEC_EXISTS=$(test -d deltaspec && echo "true" || echo "false")
```

**`MODE == "propose"` かつ `DELTASPEC_EXISTS == "false"` の場合（auto_init）**:
