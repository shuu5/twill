...
子 Issue 番号を配列に収集しながら、各抽出候補について起票。controller が `COMPONENT_NAME`, `STEP_NAME`, `DEST_PATH`, `TYPE`, `VERDICT`, `LINE_COUNT` を候補ごとに設定してループ実行する:
...
## 概要
<コンポーネント名> の <Step名> を `<抽出先パス>` に抽出する。
## 背景・動機
親 Issue: #<親Issue番号> / twl audit 結果: <Verdict>（<行数>行 / 上限80行）
## スコープ
**含む:** `<抽出先パス>` の作成（type: <型>）、元 controller から該当 Step の実装を移動、deps.yaml 更新
**含まない:** 他 Step のリファクタリング（別 Issue で対応）
## 受け入れ基準
- [ ] `<抽出先パス>` が作成されている
