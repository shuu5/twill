...
- `<context-name>`: kebab-case のコンテキスト名（`$ARGUMENTS` から取得。`--type=` を除いた残余文字列を xargs でトリムする）
- `--type=<value>`（省略可）: co-architect から引き継ぐ ProjectType（例: `ddd`, `generic`）。省略時は `architect-completeness-check.md Step 0.5` と同等の解決順序（`.architecture-type` ファイル → `architecture/vision.md` frontmatter → `ddd` フォールバック）で自動決定する。
...
1. `/twl:explore` を Skill tool で呼び出し、以下を渡す:
   ...
   - `--type` 引数（存在する場合）: `--type=<value>` として explore に伝搬する（type 伝搬）
