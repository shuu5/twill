...
test_new_path_dispatch_mentioned() {
  assert_file_exists "$SKILL_MD" || return 1
  assert_file_contains "$SKILL_MD" 'issue-lifecycle-dispatch' || return 1
  return 0
}
...
test_phase2_branch_at_start() {
  assert_file_exists "$SKILL_MD" || return 1
  assert_file_contains "$SKILL_MD" 'CO_ISSUE_V2.*1.*Phase 2|Phase 2.*CO_ISSUE_V2.*1' || return 1
  return 0
}
...
test_per_issue_in_dir_described() {
  assert_file_exists "$SKILL_MD" || return 1
  assert_file_contains "$SKILL_MD" 'per-issue.*IN|IN.*per-issue' || return 1
  return 0
}
...
# THEN: issue-lifecycle-dispatch.sh <sid> 0 --max-parallel 3 が実行され、
#       issue-lifecycle-wait.sh <sid> 0 が Bash-bg で起動される

test_dispatch_sh_called() {
  assert_file_exists "$SKILL_MD" || return 1
  assert_file_contains "$SKILL_MD" 'issue-lifecycle-dispatch\.sh' || return 1
  return 0
}

test_dispatch_max_parallel_3() {
  assert_file_exists "$SKILL_MD" || return 1
  assert_file_contains "$SKILL_MD" 'max-parallel.*3|--max-parallel.*3' || return 1
  return 0
}

test_wait_sh_background() {
  assert_file_exists "$SKILL_MD" || return 1
  assert_file_contains "$SKILL_MD" 'issue-lifecycle-wait\.sh' || return 1
  return 0
}

test_bash_background_mentioned() {
  assert_file_exists "$SKILL_MD" || return 1
  assert_file_contains "$SKILL_MD" 'run_in_background|Bash.*bg|Bash-bg' || return 1
  return 0
}
...
# Edge case: --timeout 3600 が wait スクリプトに指定
test_wait_timeout_3600() {
  assert_file_exists "$SKILL_MD" || return 1
  assert_file_contains "$SKILL_MD" 'timeout.*3600|3600.*timeout' || return 1
  return 0
}

if [[ -f "${PROJECT_ROOT}/${SKILL_MD}" ]]; then
  run_test "Phase 3: issue-lifecycle-dispatch.sh の呼び出しが記述されている" test_dispatch_sh_called
  run_test "Phase 3: --max-parallel 3 が指定されている" test_dispatch_max_parallel_3
  run_test "Phase 3: issue-lifecycle-wait.sh が記述されている" test_wait_sh_background
  run_test "Phase 3: Bash-bg (run_in_background) が記述されている" test_bash_background_mentioned
  ...
  run_test_skip "Phase 3: issue-lifecycle-dispatch.sh の呼び出しが記述されている" "skills/co-issue/SKILL.md not found"
  run_test_skip "Phase 3: --max-parallel 3 が指定されている" "skills/co-issue/SKILL.md not found"
  run_test_skip "Phase 3: issue-lifecycle-wait.sh が記述されている" "skills/co-issue/SKILL.md not found"
  run_test_skip "Phase 3: Bash-bg (run_in_background) が記述されている" "skills/co-issue/SKILL.md not found"
...
test_non_dependent_failure_continues() {
  assert_file_exists "$SKILL_MD" || return 1
  assert_file_contains "$SKILL_MD" 'warning.*継続|warning.*continue|継続.*warning|depend.*warn' || return 1
  return 0
}
...
test_done_issues_skipped_on_retry() {
  assert_file_exists "$SKILL_MD" || return 1
  assert_file_contains "$SKILL_MD" 'STATE.*done.*skip|done.*skip.*retry|resume.*done.*skip' || return 1
  return 0
}
...
test_phase5_exists_in_skill() {
  assert_file_exists "$SKILL_MD" || return 1
  assert_file_contains "$SKILL_MD" 'Phase 5|Phase\s*5' || return 1
  return 0
}
...
test_phase5_skip_when_closed() {
  assert_file_exists "$SKILL_MD" || return 1
  assert_file_contains "$SKILL_MD" 'closed.*skip|CLOSED.*skip|skip.*closed' || return 1
  return 0
}
...
test_phase5_only_runs_with_v2_flag() {
  assert_file_exists "$SKILL_MD" || return 1
  assert_file_contains "$SKILL_MD" 'CO_ISSUE_V2.*Phase 5|Phase 5.*CO_ISSUE_V2' || return 1
  return 0
}
...
test_phase5_log_includes_sid_timestamp() {
  assert_file_exists "$SKILL_MD" || return 1
  assert_file_contains "$SKILL_MD" 'session.*sid|sid.*session|YYYY-MM-DD|timestamp' || return 1
  return 0
}

if [[ -f "${PROJECT_ROOT}/${SKILL_MD}" ]]; then
  run_test "Phase 5 が SKILL.md に記述されている" test_phase5_exists_in_skill
