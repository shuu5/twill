...
test_agent_api_key_check() {
  ...
  assert_file_contains "$AGENT_FILE" "CODEX_API_KEY|OPENAI_API_KEY|config\.toml|auth\.json"
}
run_test "認証方法チェック - CODEX_API_KEY/OPENAI_API_KEY/config.toml/auth.json のいずれかが記述されている" test_agent_api_key_check
...
# Edge case: ~/.codex/auth.json も認証方法として記述されている
test_agent_auth_json_check() {
  assert_file_exists "$AGENT_FILE" || return 1
  assert_file_contains "$AGENT_FILE" "auth\.json"
}
run_test "graceful skip [edge: ~/.codex/auth.json が認証方法として記述されている]" test_agent_auth_json_check
