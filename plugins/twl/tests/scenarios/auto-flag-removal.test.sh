...
test_autopilot_launch_prompt_issue_only() {
  # /twl:workflow-setup #${ISSUE} 形式（フラグなし）が .sh に存在する
  assert_file_contains "$AUTOPILOT_LAUNCH_SH" "workflow-setup #\\\$\{?ISSUE\}?"
}

if [[ -f "${PROJECT_ROOT}/${AUTOPILOT_LAUNCH_SH}" ]]; then
  run_test "autopilot-launch のプロンプトが /twl:workflow-setup #\${ISSUE} 形式である" test_autopilot_launch_prompt_issue_only
elif [[ -f "${PROJECT_ROOT}/${AUTOPILOT_LAUNCH}" ]]; then
  run_test "autopilot-launch のプロンプトが /twl:workflow-setup #\${ISSUE} 形式である" test_autopilot_launch_prompt_issue_only
else
  run_test_skip "autopilot-launch のプロンプトが /twl:workflow-setup #\${ISSUE} 形式である" "autopilot-launch not found"
fi
...
if [[ -f "${PROJECT_ROOT}/${C2D_SPEC}" ]]; then
  run_test "c-2d spec の Worker 起動プロンプトが /twl:workflow-setup #\${ISSUE} 形式である" test_c2d_spec_prompt_issue_format
else
  run_test_skip "c-2d spec の Worker 起動プロンプトが /twl:workflow-setup #\${ISSUE} 形式である" "${C2D_SPEC} not found"
fi
