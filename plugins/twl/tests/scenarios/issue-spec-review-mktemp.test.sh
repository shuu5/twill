...
#
# Scenarios:
#   1. manifest ファイル生成 — mktemp + chmod 600
#   2. CONTEXT_ID 導出 — basename からプレフィックス除去で 8 文字取得
...
test_context_id_derivation_from_manifest_path() {
  ...
  local manifest_path="/tmp/.specialist-manifest-AbCd1234.txt"
  local expected_context="AbCd1234"
  ...
  # 8 文字であることを確認
  if [[ ${#derived} -ne 8 ]]; then
    echo "expected 8 chars, got ${#derived} ('${derived}')" >&2
    return 1
  fi

  return 0
}
run_test "CONTEXT_ID 導出: basename からプレフィックス除去で 8 文字取得" \
  test_context_id_derivation_from_manifest_path
...
test_context_id_derivation_from_real_mktemp() {
  ...
  # 英数字 8 文字であるか確認
  if [[ ! "$context_id" =~ ^[a-zA-Z0-9]{8}$ ]]; then
    echo "CONTEXT_ID does not match [a-zA-Z0-9]{8}: '${context_id}'" >&2
    cleanup_manifest "$manifest_file"
    return 1
  fi
...
run_test "CONTEXT_ID 導出 (edge): 実際の mktemp パスから 8 文字英数字を取得できる" \
  test_context_id_derivation_from_real_mktemp
