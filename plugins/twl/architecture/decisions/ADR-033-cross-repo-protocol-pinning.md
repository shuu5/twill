...
クロスリポジトリ依存を文書化する既存の仕組み（contracts/、git submodule、lock ファイル）はいずれも「どのインターフェースに依存しているか」を人が読める形で固定点付きで記録する用途に適していなかった。特に tag/branch 参照は見た目は安定しているが実際には可変であり、drift の温床となる。commit SHA による固定点を持つ専用ディレクトリを設けることで、再現性と可視性を両立できる。
