...
Accepted (Amended 2026-05-02: Decision 5 added — Tier 2 caller migration; Amended 2026-05-08: Decision 6 added — Tier 1+ observer/controller MCP tool 6 件の設計方針)
...
| 2026-05-08 | Decision 6 (new) | Tier 1+ observer/controller bash 呼び出し MCP 化拡張の設計方針を追記 (epic #1271)。spawn/observation/budget 系 6 tool の設計方針・shadow rollout pattern・既存実装との差分を記述。あわせて Decision 5 および `architecture/migrations/tier-2-caller/migration-strategy.md §2.3` の `exit 2` 誤記を `exit 1` に修正（実装との整合）。 |
...
| **Phase 2**: shadow migration | production caller 5 ファイル ~15 行を `session_msg send` API に書換、両 backend 並走（mcp は dry-run）、shadow log で MCP/tmux 整合性比較 | `TWILL_MSG_BACKEND=mcp_with_fallback`（log only） | shadow log で 1 週間以上 mismatch 0 件確認、`mcp-shadow-compare.sh` PASS（exit 0 = mismatch 0、exit 1 = mismatch あり） |
| **Phase 3**: blocking 切替 | default を mcp に切替、`tmux send-keys` fallback は kill-window 等の緊急介入のみ。merge 時に `#1033` close + `#1034` epic close（Tier B `#1032` merge と AND） | `TWILL_MSG_BACKEND=mcp`（default） | bats integration test（Pilot↔Worker 双方向、AT 非依存性 = `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=0` 確認）PASS |
| **Phase 4**: cleanup | `tmux send-keys` 直叩き lint を `twl audit` に追加、`session-comm.sh::cmd_inject_file` 削除、`#1050`/`#1197` 再評価・close 判定 | — | `tmux send-keys` lint PASS（whitelist 例外のみ）、`#1050` close 完了（`#1033`/`#1034` は Phase 3 で close 済） |
...
- AC5-6: `#1033` close（Phase 3 完遂時、本 ADR Decision 5 §決定 item 4 と Decision 3 §#1034 close 条件に整合）、`#1034` epic close（Phase 3 完遂時 + Tier B `#1032` merge の AND）、`#1050` close（Phase 4 完了時、cmd_inject_file 削除の系として）
...
- AC5-9: 各 Phase の PR で `twl check --deps-integrity` PASS（chain SSoT との非干渉を機械検証、ADR-022 整合保証）

#### Bounded Context 整合

- caller migration は **Autopilot Context** および **Live Observation Context** 配下の `plugins/session/` 通信プリミティブ層（`session-comm.sh`）内部の I/O 経路変更のみ。`plugins/session/` は独立 Bounded Context ではなく、両 Context が共有するインフラ層 (Shared Kernel) として位置付け
...
- ADR-022 (chain-ssot-boundary) との整合: `session_msg` API 追加は `chain.py CHAIN_STEPS` / `chain-steps.sh` / `deps.yaml.chains` の 3 SSoT に直接影響しない（通信プリミティブ層のため）が、Phase 1-4 の各 PR で `twl check --deps-integrity` PASS を機械検証要件 (AC5-9) として明示する
...
### Decision 6: Tier 1+ observer/controller MCP tool 化拡張（2026-05-08 amendment）

#### 背景（Decision 6）

Wave 21 の Tier 2 caller migration（Decision 5）完遂後の次層改善として、observer / controller の自律性を制約する bash 呼び出し依存を解消する。observer は 5 軸でシェルコマンドに依存しており、30s ポーリング・inject 失敗 10%・flock 競合（#1050）・memory_store 漏れ等の構造的問題を抱える。本 Decision は Tier 1+ として 6 tool を追加し、event-driven 化への道筋を確立する（epic #1271）。

| 軸 | 現状 (bash) | 本 Decision 完遂後 (MCP) |
|----|-------------|------------------------|
| state polling | `tmux capture-pane` + ANSI 解析（30s 周期） | `twl_get_session_state` on-demand 構造化 JSON |
| inject 検知 | "Press up to edit" regex polling（FP ~10%） | `twl_get_pane_state` 構造化判定 |
| AskUserQuestion proxy | input-waiting regex + `tmux send-keys` | mailbox subscription + `twl_recv_msg` |
| hook race | `tmux load-buffer` flock 競合（#1050） | MCP tool 内 atomic dispatch |
| memory_store 漏れ | LLM 任意（PostCompact / Stop hook 不在） | `twl_state_write` で auto-stamp |

#### 6 tool 設計方針（Decision 6）

以下 6 tool を `cli/twl/src/twl/mcp_server/tools.py` に追加する（各 tool の個別実装は子 Issue 6 件、epic #1271 配下）。

| # | Tool | Wrapped 対象 | 分類 |
|---|------|-------------|------|
| 1 | `mcp__twl__twl_spawn_session` | `plugins/session/scripts/cld-spawn` | spawn 系 |
| 2 | `mcp__twl__twl_spawn_controller` | `plugins/twl/skills/su-observer/scripts/spawn-controller.sh` | spawn 系 |
| 3 | `mcp__twl__twl_capture_pane` | `tmux capture-pane` → 構造化 JSON（ANSI strip + raw 2 mode） | observation 系 |
| 4 | `mcp__twl__twl_list_windows` | `tmux list-windows/sessions` → 構造化 JSON | observation 系 |
| 5 | `mcp__twl__twl_get_session_state` | `plugins/session/scripts/session-state.sh`（既存 `twl_get_session_state` を拡張・置換） | observation 系 |
| 6 | `mcp__twl__twl_get_budget` | `tmux capture-pane` + regex で 5h/7d budget extract | budget 系 |

**分類別設計判断**:
- **spawn 系**（tool 1, 2）: side-effect 大のため shadow log は wrapped bash の exit code と stderr 構造化記録のみ。blocking 切替は Wave 22 後半の別 Issue（本 epic の完了条件に含まない）。ADR-026 (spawn-syntax-discipline) の WARN 機構との関係: MCP tool 経由 spawn は `spawn-controller.sh --with-chain` を内部で呼び出すため ADR-026 の skill bypass 検知対象外（呼び出し元が tool 内部のため shell audit から見えない）。この特性は子 Issue 設計時に明記すること
- **observation 系**（tool 3, 4, 5）: ANSI strip + state field 抽出 → 構造化 JSON 返却。既存 `twl_get_pane_state` / `twl_get_session_state` との責務境界を子 Issue で明確化
- **budget 系**（tool 6）: Anthropic side format change 対応のため `refs/ref-budget-format.md` で format spec を pin
- **#754 教訓継承**: 全 6 tool は short-lived（1 秒未満）。spawn 系のみ例外だが fire-and-forget で deadlock 回避

#### shadow rollout pattern（Decision 6）

Wave 21（Decision 5）で validated 済の 3-step migration を全 6 tool に踏襲する（#1225 pattern）:

1. **Step 1**: bash hook 保持 + mcp_tool log only 並走、`mcp-shadow-compare.sh` で整合検証
2. **Step 2**: regression なし確認（1 週間以上、mismatch 0 件）後 blocking 切替。`mcp-shadow-compare.sh` PASS（exit 0 = mismatch 0、exit 1 = mismatch あり）
3. **Step 3**: 既存 bash hook 削除

spawn 系（tool 1, 2）は Step 1 まで（blocking 切替は別 Issue）。observation/budget 系（tool 3-6）は 3 step 全て完遂（子 Issue 設計で詳細化）。

#### 既存実装との差分（Decision 6）

- `twl_get_session_state`（tool 5）: `tools.py` line ~218 に同名 tool が既存実装済み。子 Issue で既存実装との差分・置換方針を明確化すること（機能重複ではなく拡張・統合の方向で設計）
- `twl_get_pane_state`（observation 系の基盤）: `tools.py` に既存実装あり。tool 3 (`twl_capture_pane`) との責務境界を子 Issue で明確化
- AT 非依存性要件: 全 6 tool の子 Issue に `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=0` での動作確認を AC として継承（Decision 5 AC5-3 pattern 踏襲）

#### 不採用案（Decision 6 内サブ案）

- **案 6A (Tier 2 完遂前に Tier 1+ 着手)**: Wave 21 の Tier 2 caller migration 完遂前に並行して Tier 1+ 実装を開始する案。**不採用理由**: Tier 2 migration 中に Tier 1+ の MCP tool 群を追加すると tools.py の変更が競合し、session 通信経路の shadow log 解析が複雑化する。Wave 21 の AC5-5（mismatch 0 件 1 週間維持）が確定するまで Tier 1+ を保留する（epic #1271 AC5 として明記）
- **案 6B (bash hook を残存させたまま全 6 tool を blocking 切替)**: shadow mode を省略して全 6 tool を一括 blocking 切替する案。**不採用理由**: spawn 系（tool 1, 2）は side-effect が大きく、regression リスクが高い。Decision 5 / #1225 で validated 済の 3-step pattern を放棄する根拠なし

#### Acceptance Criteria（Decision 6 / epic #1271 の前提整備）

- AC6-1: 本 ADR Decision 6 セクション存在（本 amendment で充足）
- AC6-2: `exit 2` 誤記が `exit 1` に修正済み — ADR-029 Decision 5 Phase 2 テーブルおよび `architecture/migrations/tier-2-caller/migration-strategy.md §2.3`（本 amendment で充足）
- AC6-3: epic #1271 AC3 達成証明として PR description に ADR-029 diff link を貼付

### Decision 6 関連の Consequences（2026-05-08 amendment）

#### Positive (Decision 6)

- **observer cycle 構造的短縮**: `twl_get_session_state` on-demand 化により capture-pane 30s ポーリングを排除。推定 ~83% 短縮（30s → ~10s/cycle）
- **inject 失敗率解消見込み**: `twl_get_pane_state` 構造化判定により "Press up to edit" regex false positive（FP ~10%）が ~0% に改善
- **shadow mode pattern の再利用**: #1225 / Decision 5 で validated 済の 3-step migration を踏襲、新規リスク導入なし
- **AT 非依存性保証**: 全 6 tool に `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=0` 動作確認を AC として継承

#### Negative (Decision 6)

- **spawn 系のblocking 切替遅延**: tool 1, 2 は side-effect 大のため Wave 22 後半の別 Issue まで blocking 切替が延期される
- **tools.py の肥大化**: 6 tool 追加で tools.py が ~1000 行超えに近づく可能性。Decision 5 §tools.py モジュール分割評価と合わせて Wave 22 で判定
- **ADR-026 bypass リスク**: spawn 系 tool（1, 2）は MCP tool 内部から spawn を呼び出すため ADR-026 の shell audit が届かない。子 Issue で代替検証方法を設計する必要あり

#### Mitigations (Decision 6)

- **段階的実装**: spawn 系は Step 1（log only）止まり、observation/budget 系は完全 3-step — リスク段階に応じたスコープ分離
- **metrics 計測 AC**: AC6 系（WARNING）として observer cycle 短縮・inject 失敗率計測仕様を子 Issue に記載（epic #1271 AC6/AC7 参照）
- **tools.py 分割計画参照**: Decision 5 §tools.py モジュール分割評価を Wave 22 子 Issue 1 で判定する既存計画と連携
...
- **緊急 rollback の永続化操作必要**: blocking 切替後（Phase 3）の即時復帰は `export TWILL_MSG_BACKEND=tmux` の 1 コマンドで成立する（caller 側コードは `session_msg` API 経由のため無変更、`rollback-plan.md` §2.1 参照）が、再起動・新 session を含む全プロセスへの永続化は `~/.bashrc` / tmux session hook / `.envrc` 等への env var 設定が追加で必要であり、純粋な oneshot コマンドではセッションスコープに限定される
