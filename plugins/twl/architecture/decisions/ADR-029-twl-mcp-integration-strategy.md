...
| **Phase 2**: shadow migration | production caller 5 ファイル ~15 行を `session_msg send` API に書換、両 backend 並走（mcp は dry-run）、shadow log で MCP/tmux 整合性比較 | `TWILL_MSG_BACKEND=mcp_with_fallback`（log only） | shadow log で 1 週間以上 mismatch 0 件確認、`mcp-shadow-compare.sh` PASS（exit 0 = mismatch 0、exit 2 = mismatch あり） |
