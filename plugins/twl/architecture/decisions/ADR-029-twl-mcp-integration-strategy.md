...
| 5 | 通信系 tool 群追加（mailbox MCP hub） | comm | #1037 Tier 2 + #1034 Tier C 案 A | 子 1, 2 完了後 |
...
- **close 条件**: 新統合 epic の autopilot 系 (mergegate/orchestrator/worktree) 子 Issue 群が全 merge された段階
...
### Decision 4: 自律実行のための優先順位

新統合 epic の子 Issue 起票後、co-autopilot で自律実行可能な順序:

```
Wave 1: 子 1 (棚卸し + 拡充計画)         — 1 Pilot 1 Issue
Wave 2: 子 2 (検証系) + 子 3 (状態系) + 子 4 (autopilot 系第 1 Wave)  — 3 並走
Wave 3: 子 5 (通信系)                     — 1 Pilot 1 Issue
```

子 1 は co-explore 経由で「拡充計画」を策定する性質上、**co-autopilot ではなく co-explore + co-issue で扱う**（実装ではなく設計タスク）。子 2-5 は通常の co-autopilot 実装フロー。
