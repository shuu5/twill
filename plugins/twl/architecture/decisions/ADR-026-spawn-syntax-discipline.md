...
再評価のトリガー条件（**担当: Pilot**、判定タイミング: Phase AB 開始時 Issue 起票で確認）:
- Pilot-internal 呼出経路が `autopilot-launch.sh` 直接呼出しから `spawn-controller.sh --with-chain` 経由に変更される場合
- または `spawn-controller.sh --with-chain` を廃止したい場合

上記トリガーが発火した際に以下を順に検討する:

- **rename（`--direct-worker-launch`）**: Pilot-internal 呼出経路の分離が完了した段階で rename を検討。完了の判定基準: `autopilot-launch.sh` が `spawn-controller.sh --with-chain` を使用しないことを bats テストで検証済みであること
- **deprecate（exit 2 化）**: rename 後、旧 `--with-chain` を使うケースがゼロになった段階で段階的に deprecate
