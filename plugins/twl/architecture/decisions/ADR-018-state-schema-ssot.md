...
**Issue**: #507  
**Supersedes**: —  
**Related**: ADR-003 (unified state file)、ADR-021 (pilot-driven workflow loop)、ADR-023 (deltaspec-free chain と TDD 直行 flow — `post-change-apply` terminal step の削除を定義)
...
> **注記 (2026-04-23, ADR-023)**: `post-change-apply` は [ADR-023: deltaspec-free chain と TDD 直行 flow](ADR-023-tdd-direct-flow.md) D-1 で DeltaSpec 5 step とともに chain から除去される。Wave B (#Z-CORE) 以降、本テーブルからも `post-change-apply` 行を削除し、`workflow-pr-verify` への inject trigger は後続 TDD 直行 flow の terminal step に再割当する (ac-scaffold-tests 終点 or 実装終端 step)。Wave A 時点では実装上 `post-change-apply` が残存するため本テーブル行は維持する。
