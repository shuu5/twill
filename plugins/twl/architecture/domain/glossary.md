...
| ★HUMAN GATE | ユーザーの判断・承認が必要な箇所を示す統一マーカー (ADR-029)。主に observer の Layer 1/2 介入境界を明示、副次的に autopilot 系の AskUserQuestion ポイントもマーク。grep で hand-off ポイントを集約可能。AUTO モード bypass の自動承認には付与しない | 全体 (observer 主体) |
