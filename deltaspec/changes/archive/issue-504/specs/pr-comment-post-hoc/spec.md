...
- `plugins/twl/scripts/autopilot-launch.sh` の L84（`--autopilot-dir` 引数パース）/ L217（`export AUTOPILOT_DIR`）/ L309（`AUTOPILOT_ENV` 変数構築）/ L366（`tmux new-window` への env 引き渡し）の 4 箇所すべてに言及する
...
#### Scenario: 重複投稿防止
- **WHEN** PR #501 のコメント一覧を取得したとき、`[post-hoc note]` プレフィックスを含む既存コメントが存在する（body の先頭が `[post-hoc note]` で始まる）
