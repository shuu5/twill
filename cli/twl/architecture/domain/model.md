...
### Change（独立集約）
deltaspec/changes/<name>/ に対応。Plugin とは独立したライフサイクルを持つ。
...
| TypeName | "controller" \| "supervisor" \| "workflow" \| "atomic" \| "composite" \| "specialist" \| "reference" \| "script" | コンポーネント型名 |
...
**Spec Management** は独立した Context（他との依存なし）。deltaspec/ ディレクトリを操作対象とし、Plugin Structure とはデータを共有しない。
