...
twl CLI は Claude Code プラグインの構造検証・可視化ツールとして開発された。
配布先は多様な開発環境（ホスト、コンテナ、CI）であり、依存関係の管理コストを最小化する必要があった。

検討した選択肢:
1. **単一ファイル（twl-engine.py）**: 外部依存最小、コピーだけでデプロイ可能
2. **パッケージ構造（src/twl/）**: モジュール分割による保守性向上、pip install でデプロイ
3. **Go/Rust バイナリ**: シングルバイナリ配布、高速実行

## Decision

Python 単一ファイル（twl-engine.py）で全機能を実装する。

理由:
- Claude Code プラグインのエコシステムが Python スクリプトを前提としている
- `python3 twl-engine.py` だけで動作し、pip install やビルドステップが不要
...
**良い点:**
- デプロイが `cp twl-engine.py /target/` で完結
- CI/コンテナ環境で追加セットアップ不要
- twl ラッパースクリプト経由でパス解決を抽象化
...
- twl 自身の `--audit` / `--complexity` で自己検証可能
