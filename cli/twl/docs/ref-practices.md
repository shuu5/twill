...
各型のトークン上限（値は types.yaml から導出。`twl --sync-check <ref-practices.md>` で整合性検証可能）。

| 型 | Warning | Critical | 備考 |
|---|---|---|---|
| controller | 1,500 tok | 2,500 tok | |
| workflow | 1,200 tok | 2,000 tok | |
| atomic | 1,500 tok | 2,500 tok | composite と共通 |
| composite | 1,500 tok | 2,500 tok | atomic と共通 |
| specialist | 1,800 tok | 2,500 tok | |

reference・script は token_target なし（意図的除外）。
