...
class TestAC1SkeletonImplemented:
    ...
    def test_ac1_has_function_main(self):
        # AC: ac8_run_240_trials.py に main() 関数が実装されている
        # RED: ファイル未存在のため FAIL
        ...
        assert hasattr(module, "main"), "main() 関数が定義されていない"
        assert callable(module.main), "main が callable でない"

    def test_ac1_has_function_run_trial(self):
        # AC: ac8_run_240_trials.py に run_trial() 関数が実装されている
        # RED: ファイル未存在のため FAIL
        if not AC8_RUN_SCRIPT.exists():
            raise NotImplementedError("AC #1 未実装: ac8_run_240_trials.py が存在しない")
        spec = importlib.util.spec_from_file_location("ac8_run_240_trials", AC8_RUN_SCRIPT)
        module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(module)
        assert hasattr(module, "run_trial"), "run_trial() 関数が定義されていない"
        assert callable(module.run_trial), "run_trial が callable でない"

    def test_ac1_has_function_classify_failure(self):
        # AC: ac8_run_240_trials.py に classify_failure() 関数が実装されている
        # RED: ファイル未存在のため FAIL
        if not AC8_RUN_SCRIPT.exists():
            raise NotImplementedError("AC #1 未実装: ac8_run_240_trials.py が存在しない")
        spec = importlib.util.spec_from_file_location("ac8_run_240_trials", AC8_RUN_SCRIPT)
        module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(module)
        assert hasattr(module, "classify_failure"), "classify_failure() 関数が定義されていない"
        assert callable(module.classify_failure), "classify_failure が callable でない"

    def test_ac1_has_function_goldfile_match(self):
        # AC: ac8_run_240_trials.py に goldfile_match() 関数が実装されている
        # RED: ファイル未存在のため FAIL
        if not AC8_RUN_SCRIPT.exists():
            raise NotImplementedError("AC #1 未実装: ac8_run_240_trials.py が存在しない")
        spec = importlib.util.spec_from_file_location("ac8_run_240_trials", AC8_RUN_SCRIPT)
        module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(module)
        assert hasattr(module, "goldfile_match"), "goldfile_match() 関数が定義されていない"
        assert callable(module.goldfile_match), "goldfile_match が callable でない"
...
class TestAC2CldMcpPoC:
    ...
    def test_ac2_run_trial_with_mcp_route_succeeds(self):
        # AC: run_trial() を mcp 経路で実行した場合、success=True が返る
        # RED: ac8_run_240_trials.py 未実装のため FAIL
        if not AC8_RUN_SCRIPT.exists():
            raise NotImplementedError("AC #2 未実装: ac8_run_240_trials.py が存在しない")
        spec = importlib.util.spec_from_file_location("ac8_run_240_trials", AC8_RUN_SCRIPT)
        module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(module)
        result = module.run_trial(operation="issue_init", route="mcp", trial_index=0)
...
class TestAC5SmokeSuccessRate:
    ...
    def test_ac5_goldfile_match_function_works(self):
        ...
        module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(module)
        # goldfiles の1件を使って goldfile_match が動作することを確認
        ...
        result = module.goldfile_match(output=expected, operation="issue_init", route="cli")
