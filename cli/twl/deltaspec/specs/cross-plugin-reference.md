...
- **THEN** twl はこれを session plugin の session-state コンポーネントへの cross-plugin 参照として認識する
...
`twl validate` は cross-plugin 参照について、参照先 plugin の deps.yaml を読み込み、型整合性を検証しなければならない（MUST）。参照先 plugin が見つからない場合は warning を出力してスキップしなければならない（SHALL）。
...
`twl check` は cross-plugin 参照先のファイル存在を検証しなければならない（MUST）。参照先 plugin が見つからない場合は warning を出力してスキップしなければならない（SHALL）。
