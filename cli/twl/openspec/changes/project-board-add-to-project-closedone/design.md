...
loom リポジトリの Issue を GitHub Projects (loom-dev-ecosystem, Project #3) と連携させる。`actions/add-to-project@v1` Action と `gh api graphql` による Status 更新の 2 つの workflow を配置する。PAT Secret (`ADD_TO_PROJECT_PAT`) は loom-plugin-dev#114 で一括登録される前提。
...
- PAT の権限スコープ: `project` スコープが必要。Secret 未登録時は workflow が失敗するが、想定内（loom-plugin-dev#114 の完了が前提）。
