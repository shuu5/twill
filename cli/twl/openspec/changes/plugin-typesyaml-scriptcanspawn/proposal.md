...
- `twl validate` が cross-plugin 参照を検証（参照先 plugin の deps.yaml を解決）
- `twl check` が cross-plugin 参照先のファイル存在を検証
...
- **Cross-plugin 参照検証**: `twl validate` が参照先 plugin の deps.yaml を解決し、型整合性を検証
- **Cross-plugin ファイル存在検証**: `twl check` が参照先 plugin 内のファイル存在を確認
...
- **twl-engine.py**: `validate_types()`, `validate_body_refs()`, check 関連関数に cross-plugin 解決ロジックを追加
