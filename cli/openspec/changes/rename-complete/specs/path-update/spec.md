...
- **WHEN** `loom rename controller-project co-project` を実行し、対象の path が `skills/controller-project/SKILL.md` である
- **THEN** path は `skills/co-project/SKILL.md` に更新される

#### Scenario: 部分一致しない
- **WHEN** `loom rename co-auto co-autopilot` を実行し、別コンポーネントの path に `skills/co-autopilot-launch/SKILL.md` がある
