...
- **WHEN** `loom rename controller-project co-project` を実行し、entry_points に `skills/controller-project/SKILL.md` が含まれる
- **THEN** entry_points の該当エントリが `skills/co-project/SKILL.md` に更新される

#### Scenario: entry_points が未定義
- **WHEN** `loom rename some-cmd new-cmd` を実行し、deps.yaml に entry_points キーが存在しない
- **THEN** エラーなく正常に完了する（entry_points 更新はスキップされる）

#### Scenario: dry-run での entry_points 変更表示
- **WHEN** `loom rename controller-project co-project --dry-run` を実行し、entry_points に該当パスが含まれる
