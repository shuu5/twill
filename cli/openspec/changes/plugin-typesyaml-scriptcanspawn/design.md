...
loom は Claude Code プラグインの構造定義・検証・可視化を行う CLI ツール。各 plugin は `deps.yaml` でコンポーネント間の依存関係を宣言し、`types.yaml` の型ルール（can_spawn/spawnable_by）に基づいて整合性を検証する。
...
- `loom validate` が cross-plugin 参照の型整合性を検証する（参照先 plugin の deps.yaml を読み込み）
- `loom check` が cross-plugin 参照先のファイル存在を検証する
