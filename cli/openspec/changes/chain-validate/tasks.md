...
- [x] 1.1 `chain_validate(deps, plugin_root)` 関数のスケルトンを `loom-engine.py` に追加（`deep_validate` の直後、同じシグネチャ `(criticals, warnings, infos)` を返す）
...
- [x] 4.6 loom check 統合テスト（v3.0 / v2.0 分岐確認）
