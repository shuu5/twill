...
- `loom-engine.py` に `chain_validate(deps, plugin_root)` 関数を追加
...
- `loom check` 実行時に v3.0 deps.yaml を検出した場合に自動実行
...
- **loom check**: v3.0 deps.yaml 検出時に chain 検証を自動統合

## Impact

- **変更ファイル**: `loom-engine.py`（chain_validate 関数追加 + check コマンド統合）
- **テスト**: `tests/` に chain 検証用テストケース追加
- **CLI**: `loom` ラッパーへの変更なし（engine 内で統合）
