...
twl-engine.py の deep_validate は (A) controller-bloat, (B) ref-placement, (C) tools-mismatch の3カテゴリを検証している。audit_report は 5セクション構成。specialist の model フィールドは現在いずれでも未検証。

Issue #29 で specialist 型の model 宣言チェックを追加する。許可値は twl-engine.py 内定数として定義し、types.yaml には含めない。
...
2. **ALLOWED_MODELS の位置**: twl-engine.py モジュールレベル定数。`{"haiku", "sonnet", "opus"}`
...
- ALLOWED_MODELS がハードコードのため、新モデル追加時に twl-engine.py の更新が必要。ただし未知モデルは INFO（非ブロック）なので実運用への影響は軽微
