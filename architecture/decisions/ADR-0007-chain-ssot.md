...
| 値 | 意味 |
|---|---|
| `runner` | chain-runner（Python または bash）が機械的に実行。CHAIN_STEPS に含まれる |
| `llm` | LLM（Skill tool）が実行。chain-runner は記録のみ。CHAIN_STEPS に含まれなくてよい |
| `trigger` | 外部トリガー/スクリプトで実行。chain-runner 外部。|
